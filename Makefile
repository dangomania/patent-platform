...
STACK_NAME=patent-platform-lovedigression-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://patent-platform.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t collective/patent-platform-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t collective/patent-platform-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name patent-platform-frontend-acceptance --link patent-platform-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/patent-platform-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name patent-platform-backend-acceptance -d collective/patent-platform-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop patent-platform-frontend-acceptance
	@docker stop patent-platform-backend-acceptance
